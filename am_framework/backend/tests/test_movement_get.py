...
class MovementTableGetTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.aircrafts.append(aircrafttable.objects.create(
            tailNumber='111', aircraftType='A320', status='Departured',
            location='LAX', userId=cls.rootUser))
        cls.aircrafts.append(aircrafttable.objects.create(
            tailNumber='222', aircraftType='A330', status='Departured',
            location='ORD', userId=cls.rootUser))
        ...
        cls.movements = []
        cls.movements.append(movementtable.objects.create(
    ...
    def compareHelper(self, entryGot: dict, entryCreated: movementtable):
        ...
        self.assertEqual(entryGot['arrivalAirportId'],
                         entryCreated.arrivalAirportId)
        self.assertEqual(entryGot['originAirportId'],
                         entryCreated.originAirportId)
        ...
        self.assertEqual(entryGot['aircraftId'],
                         entryCreated.aircraftId.aircraftId)
    ...
    def test_movement_get_single_entry_with_nonPrimaryKey(self):
        ...
        response = self.client.get(
            f'/api/movement/?arrivalDate=2022-10-01T12:00:00Z')
        # Check if we get only one entry
        data = response.json()['data']
        self.assertEqual(len(data), 1)
    ...
    def test_movement_get_multiple_entries_with_nonprimaryId(self):
        ...
        response = self.client.get(
            f'/api/movement/?userId={self.rootUser.userId}')
        # Check if we get only one entry
        data = response.json()['data']
        self.assertEqual(len(data), 2)
    ...
    def test_movement_get_entries_with_multiple_fields(self):
        ...
        for i, currMovement in enumerate(self.movements):
            url = (f'/api/movement/?movementId={currMovement.movementId}'
