...
class AircraftAPIViewTestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.aircraft1 = aircrafttable.objects.create(
            tailNumber='A12345', aircraftType='A320', status='Operational', location='JFK', userId=cls.user)
        cls.aircraft2 = aircrafttable.objects.create(
            tailNumber='B67890', aircraftType='B737', status='Maintenance', location='LAX', userId=cls.user)

        # URL for AircraftTableView
        cls.url = reverse('aircraft-list')  # Use the actual name you have given to the URL in your urls.py
    ...
    def test_create_aircraft(self):
        ...
        """
        data = {'tailNumber': 'C54321', 'aircraftType': 'A330', 'status': 'Operational', 'location': 'ORD'}
    ...
    def test_update_aircraft(self):
        ...
        response = self.client.put(reverse('aircraft-detail', args=[self.aircraft1.pk]), data, format='json')
    ...
    def test_delete_aircraft(self):
        ...
        """
        response = self.client.delete(reverse('aircraft-detail', args=[self.aircraft2.pk]))
...
class FrontendReadOnlyAPIViewTestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user = User.objects.create_user(username='testuser', password='testuserpassword')
        ...
        cls.sample_user_profile = userprofile.objects.create(
            username='sampleuser', password='samplepass', role=userprofile.roleChoice.ADMIN)
    ...
    def test_post_user_profile(self):
        ...
        """
        data = {'username': 'newuser', 'password': 'newpass', 'role': userprofile.roleChoice.CORPORATE}
        response = self.client.post(self.url, data, format='json')
    ...
    def test_put_user_profile(self):
        ...
        response = self.client.put(reverse('userprofile-detail', args=[self.sample_user_profile.pk]), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
    ...
    def test_delete_user_profile(self):
        ...
        """
        response = self.client.delete(reverse('userprofile-detail', args=[self.sample_user_profile.pk]))
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)