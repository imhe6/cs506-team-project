...
class AircraftTableGetTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.aircrafts.append(aircrafttable.objects.create(
            tailNumber='222', aircraftType='A320', status='Arrived', 
            location='ORD', userId=cls.rootUser))
        cls.aircrafts.append(aircrafttable.objects.create(
            tailNumber='333', aircraftType='A330', status='Departured', 
            location='ORD', userId=cls.rootUser))
    ...
    def compareHelper(self, aircraftGot: dict, aircraftCreated: aircrafttable):
        ...
        self.assertEqual(aircraftGot['aircraftType'],
                         aircraftCreated.aircraftType)
    ...
    def test_aircraft_get_single_entry_with_nonprimaryId(self):
        ...
        for i in range(len(self.aircrafts)):
            ...
            response = self.client.get(
                f'/api/aircraft/?tailNumber={(i+1)*111}')
            # Check if we get only one entry
            data = response.json()['data']
            self.assertEqual(len(data), 1)
    ...
    def test_aircraft_get_entries_with_multiple_fields(self):
        ...
        for i, currAircraft in enumerate(self.aircrafts):
            url = (f'/api/aircraft/?aircraftId={currAircraft.aircraftId}'
