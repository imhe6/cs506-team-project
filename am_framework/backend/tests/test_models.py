...
class AircraftTableTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.aircraft1 = aircrafttable.objects.create(
            tailNumber='A12345', status='Operational', location='JFK', userId=cls.test_user)
        cls.aircraft2 = aircrafttable.objects.create(
            tailNumber='B67890', status='Maintenance', location='LAX', userId=cls.test_user)
...
class AirportTableTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.test_user = User.objects.create_user(username='airportuser', password='testpassword456')

        cls.airport1 = airporttable.objects.create(
            airportCode='LAX', latitude='33.9416', longitude='-118.4085', numAircraft=5, userId=cls.test_user)
        cls.airport2 = airporttable.objects.create(
            airportCode='JFK', latitude='40.6413', longitude='-73.7781', numAircraft=8, userId=cls.test_user)
...
class MovementTableTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.test_user = userprofile.objects.create(username='testmovementuser', password='password', role=userprofile.roleChoice.ADMIN)

        cls.airport1 = airporttable.objects.create(
            airportCode='AAA', latitude='34.0522', longitude='-118.2437', numAircraft=10, userId=cls.test_user)
        cls.airport2 = airporttable.objects.create(
            airportCode='BBB', latitude='40.7128', longitude='-74.0060', numAircraft=5, userId=cls.test_user)
        cls.aircraft = aircrafttable.objects.create(
            tailNumber='N12345', aircraftType=aircrafttable.aType.A320, status='Operational', location='AAA', userId=cls.test_user)
        cls.movement = movementtable.objects.create(
            arrivalAirportId=cls.airport1, originAirportId=cls.airport2, arrivalDate=timezone.now(), departureDate=timezone.now() - timezone.timedelta(days=1), aircraftId=cls.aircraft, userId=cls.test_user)
...
class UserProfileTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user1 = userprofile.objects.create(username='user1', password='pass1234', role=userprofile.roleChoice.ADMIN)
        cls.user2 = userprofile.objects.create(username='user2', password='pass5678', role=userprofile.roleChoice.FACILITY)
